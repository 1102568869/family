    ...
    <div class="hello">
        <el-form :model="bill" :rules="rules" ref="billForm" label-width="100px" style="width: 600px">
            <el-form-item label="金额" prop="money">
                <el-input placeholder="请输入内容" v-model.number="bill.money" auto-complete="off">
            ...
            <el-form-item label="事项" prop="name">
                <el-input placeholder="请输入内容" v-model="bill.name" clearable>
                    <el-select v-model="bill.type" slot="prepend" placeholder="请选择类型" style="width: 120px;">
                        <el-option
                                v-for="type in types"
                                :key="type.id"
                                :label="type.name"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            change1() {
                console.info(this.bill.balance)
            },
            onSubmit(formName) {
                var vm = this;
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8888/bill/post/update', this.bill)
                            .then(function (response) {
        ...
        beforeRouteEnter(to, from, next) {
            ...
            next((vm) => {
                ...
                axios.get('http://localhost:8888/bill/get/item?id=' + id)
                    .then(function (response) {
                        vm.bill = response.data;
                        vm.bill.recordtime=new Date(vm.bill.recordtime);
                        console.info("!!!!!!!!!!!!!!!!");
